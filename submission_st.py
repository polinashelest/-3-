  ...
  def getAction(self, gameState):
    """
     Возвращает действие expectimax, используя self.depth и self.evaluationFunction
       Все призраки должны быть смоделированы так, чтобы они равномерно и случайным образом выбирали из своих
       легальные ходы.
    """
    ...
    def expectimaxValue(gameState,agentIndex,currentDepth):
      #returns (value,action) tuple
      #значение по умолчанию — +inf, действие по умолчанию — ОСТАНОВИТЬ (т. е. ничего не делать)
      v=[0.0]
      nextAgent=(agentIndex+1)
